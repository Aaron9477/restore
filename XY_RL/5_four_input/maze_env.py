...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.distance_range = self.get_distance_range() # 得到无人机之间的距离范围 这个距离不能过小，像[18，20]会导致震荡
        self.stay_time = 0  # 记录无人机持续在规定范围内的时间长度
        self.done_time = 5 # 规定无人机持续在规定范围内多久才可以置done标志位
        self.title('maze')
    ...
    def reset(self):    # 每次测试初始化
        ...
        print(np.append(INPUT_RATE, distance_rate))
        return (np.append(INPUT_RATE, distance_rate))
    ...
    def step(self, action):
        ...
        distance_rate = (self.canvas.coords(self.rect)[0] - np.array(self.canvas.coords(self.oval)[0])) / TOTAL_LENGTH
        s_ = (np.append(INPUT_RATE, distance_rate))

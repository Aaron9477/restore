...
class ObjectFollower():
    def __init__(self):
        ...
        self.min_rotation_speed = rospy.get_param("~min_rotation_speed", 0.05)
    ...
    def set_cmd_vel(self, msg):
        ...
        try:
            ...
            # 减小目标区域，以免选中目标的边缘
            scaled_width = int(self.roi.width * self.scale_roi) # 乘以一个尺度
            ...
            try:
                percent_offset_x = float(target_offset_x) / (float(self.image_width) / 2.0) # x方向误差量相对于图像一半宽度的比
            except:
                percent_offset_x = 0
            ...
            if abs(percent_offset_x) > self.x_threshold:    # 如果大于阈值
                # Set the rotation speed proportional to the displacement of the target
                    ...
                    self.move_cmd.angular.z = -direction * max(self.min_rotation_speed,
                                                min(self.max_rotation_speed, abs(speed)))   # 保证角速度在允许范围内
                    self.move_cmd.linear.x = 0  # 如果不在一定范围内，线速度都设定为0
            # If we have depth values...
            else:   # 在一定范围内，才可以运行
              self.move_cmd.angular.z = 0
              if n_z:
                mean_z = float(sum_z) / n_z # 框内平均深度
                ...
                if abs(mean_z - self.goal_z) > self.z_threshold:    # 如果相对于目标在预定范围外，才会运动
                     ...
                     if speed < 0: # 后退时，速度可以快一些
                         speed *= 1.5
                         self.move_cmd.linear.x = copysign(min(self.max_linear_speed, max(self.min_linear_speed, abs(speed))), speed)   # copysign,把后一个数的符号赋给前一个数
                     else:

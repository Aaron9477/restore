...
//从ros端得到raw_image
    ...
    // ros::NodeHandle n3;
    ros::Subscriber image_get = n.subscribe("/camera/rgb/image_raw", 1, imageCallback);      
// 等待第一帧图像
    printf("after initialization, start waiting for the first image\n");
...
if(ros_enable)
{
    ...
    // ros::NodeHandle nh;        
    ros::Subscriber roi_get = n.subscribe("fasterrcnn",1, fasterrcnnCallback);
    while(!ros_recieved)
    {
        ros::spinOnce();
        // loop_rate.sleep();
...
printf("got the initializing bbox, start tracking\n");
//Remove callback
//using ros not hand selecting

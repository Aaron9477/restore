...
import sys
sys.path.append("/home/ubuntu/WYZ/py-faster-rcnn/tools")
...
#VGG16 is a very big net, so it will cost a lot of memory
    parser.add_argument('--net', dest='demo_net', help='Network to use [zf]',
                        choices=NETS.keys(), default='zf') # choose the zf network
...
if __name__ == '__main__':
    ...
    caffemodel = os.path.join(cfg.DATA_DIR, '/home/ubuntu/WYZ/py-faster-rcnn/models', NETS[args.demo_net][1])
    ...
    print("get the first image, start detect the goal!")
    #a=1.2
    #b=30
    #for i in range(cv_frame.shape[0]):
    #	for j in range(cv_frame.shape[1]):
    #        for c in range(3):
    #        	color=cv_frame[i,j][c]*a+b
    #          	if color>255:
    #                cv_frame[i,j][c]=255
    #            elif color<0:
    #                cv_frame[i,j][c]=0
    #            else:
    #                cv_frame[i,j][c]=color

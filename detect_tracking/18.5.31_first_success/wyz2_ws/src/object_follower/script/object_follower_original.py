...
class ObjectFollower():
    def __init__(self):
        ...
        self.min_linear_speed = rospy.get_param("~min_linear_speed", 0.01) 
        
        # The maximum rotation speed in radians per second
        self.max_rotation_speed = rospy.get_param("~max_rotation_speed", 0.08)
        
        # The minimum rotation speed in radians per second
        self.min_rotation_speed = rospy.get_param("~min_rotation_speed", 0.02)
        ...
        self.min_z = rospy.get_param("~min_z", 0.4)
        
        # The goal distance (in meters) to keep between the robot and the person
        self.goal_z = rospy.get_param("~goal_z", 0.7)
        ...
        try:
            ...
            # If we have depth values...
              ...
              if n_z:
                ...
                print('the depth of target is: ' + str(mean_z))                                           		
                ...
                '''if mean_z > self.min_z:
                    ...
                    if mean_z < self.max_z and (abs(mean_z - self.goal_z) > self.z_threshold):
                        ...
                        print mean_z,speed      
                         ...
                         self.move_cmd.linear.x=0 '''
                if abs(mean_z - self.goal_z) > self.z_threshold: 
                     speed = (mean_z - self.goal_z) * self.z_scale
                     if speed < 0: 
                         speed *= 1.5
                         self.move_cmd.linear.x = copysign(min(self.max_linear_speed, max(self.min_linear_speed, abs(speed))), speed) 
